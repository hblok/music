#!/usr/bin/env python3
"""
eisgang.py — "Eisgang" (~4:55, 138 BPM, F minor). Hard techno-trance in
the CIRCLE form (design notes: eisgang_notes.md; inspiration: ONE motif —
W3 "stamping feet" from Vivaldi's Winter, ../../inspiration/
Winter_Vivaldi.md — its behaviors, none of its pitches).
...
Output: /workspace/music/eisgang.wav + eisgang.mp3 (192k).
"""
...
# ------------------------------------------------------- the skyline lead
# THE WARM — chorus-only. The same seven pitches the stab has been
# hammering, sung as one connected glide line an octave below (warmth per
# review), hanging on G over the V for four bars. Hollow pulse core, two
# detuned copies, slow bloom vibrato, long dark hall.
...
clear()
LEAD_BASE = lead_line(LEAD_NOTES)
LEAD_HI = lead_line(LEAD_NOTES, transpose=12, lowpass=3400)


def place_skyline(b0, gain, hi_gain=0.0):
    add_at(lay_L, LEAD_BASE[0], bar_t(b0), gain)
    add_at(lay_R, LEAD_BASE[1], bar_t(b0), gain)
    if hi_gain > 0:
        add_at(lay_L, LEAD_HI[0], bar_t(b0), gain * hi_gain)
        add_at(lay_R, LEAD_HI[1], bar_t(b0), gain * hi_gain)
    LEAD_STMTS.append(b0)


place_skyline(B_CH1, 0.9)                          # chorus 1: alone
place_skyline(B_CH2, 0.95, hi_gain=0.30)           # chorus 2: + shimmer
place_skyline(B_FIN, 1.0, hi_gain=0.45)            # the fusion, lap 1
place_skyline(B_FIN + 16, 1.0, hi_gain=0.45)       # the fusion, lap 2
lay_L = reverb(lay_L, IR_L, 0.5)
lay_R = reverb(lay_R, IR_R, 0.5)
commit(lay_L, lay_R, 0.24)
print("skyline lead committed")
...
OUT = os.path.join(OUT_DIR, "eisgang.wav")
...
MP3 = os.path.join(OUT_DIR, "eisgang.mp3")
...
ham_cov = 100.0 * len(STAB_HAMMER_BARS) / len([b for b in CELL_BARS
                                               if BARTYPE[b] == "A"])
print(f"Skyline: sung statements {len(LEAD_STMTS)} at bars {LEAD_STMTS} "
