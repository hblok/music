#!/usr/bin/env python3
"""
morgenland_v3.py — "Morgenland" v3 (~6:12, 142 BPM, C Phrygian dom.).
THE 303 SINGS LEGATO (notes: morgenland_notes.md, v3 amendment).
V3 = V2's melody rendered like a 303 actually ties: the v2 verdict
was "very folk!" but "a broken 303" — every slid note re-attacked
(envelope + sweep restarting), ornaments were separate 53 ms blips,
slid notes overlapped. v3 renders each slide chain as ONE note with
a smoothed pitch path (one attack, one sweep, the VCA stays open),
ornaments as pitch INFLECTIONS inside that attack, terminal bends
landing audibly across the barline, detached notes gated apart
(0.85). Melody data unchanged except two slide/ornament collisions
(bar 9) resolved so one attack owns each gesture.

THE MELODY SINGS (design notes: morgenland_notes.md, v2 amendment).
The v1 listen verdict: the refrain smeared — v1 froze silver_wire's
RUN grammar (~9.5 notes/sec) and only re-pitched it; same tune-feel,
wrong dialect. THE V2 LESSON: a borrowed grammar needs a new dialect
— "the machine sings" is a statement about NOTE VALUES, not mode.

V2 REWRITE (melody only; kit, darbuka, santur voice, harmony,
structure, master all FROZEN from v1): the refrain is a SUNG hijaz
folk tune — 8 phrases of ~2 bars in 8ths/quarters/dotted values with
held phrase-endings and breath rests (79 onsets in 256 steps,
density 0.31, ~2.9 notes/sec; longest 16th run = a pickup pair).
Slides are audible portamento; notes past a quarter get a slow sung
vibrato (±0.6 %, blooming after 0.35 s). THE ROLES INVERT: the
machine SINGS, the SANTUR RUNS — all 16th-run energy lives in the
santur's answers, turnarounds and break loops. The checks invert
with it (density window, run ceiling, held fraction, breaths).
...
THE ORNAMENT VOCABULARY (folk melisma in 303 dialect, counted, and
in v2 placed where folk ornaments live — on notes >= an 8th):
  grace flick  = 32nd lower-neighbor INTO a held tone ('g' in map)
  mordent turn = main-upper 32nds on a phrase head ('m' in map)
  melisma      = slide chains bending through 3+ sung pitches

DECLARED BORROWS: the silver_wire 303 VOICE and anti-arc CUT_PROFILE
(the run grammar is NOT borrowed — the v2 lesson: that grammar is
silver_wire's identity; here it belongs to the santur only);
the maschinenherz psy engine + kit split
...
Output: /workspace/music/morgenland_v2.wav + morgenland_v2.flac.
"""
...
# THE REFRAIN v2 — 16 bars, A SUNG HIJAZ FOLK TUNE (the v2 lesson:
# note values, not runs). 8 phrases of ~2 bars: a call rising through
# the hijaz tetrachord to a held tone, its answer falling home;
# breath rests between phrases; ornaments ON the held notes; slides
# as portamento/melisma. Written as literal events
# (midi, d16, accent, slide_to, orn) — accents mark phrase heads.
# Bars 8 and 16 are the v1 landmarks, unchanged: the Db5 hang
# (entered by portamento) and the held Db->C close.

SONG_BARS = [
    # bar 1 — P1 call: C-Db rises to a held E (the aug-2nd crossing)
    [(60, 3, 1, None, "mordent"), (61, 1, 0, None, None),
     (64, 8, 0, None, "flick"), (None, 2, 0, None, None),
     (64, 1, 0, None, None), (65, 1, 0, None, None)],
    # bar 2 — P1 answer: falls through Db to a held C; breath
    [(64, 2, 1, 61, None), (61, 2, 0, None, None),
     (60, 8, 0, None, "flick"), (None, 4, 0, None, None)],
    # bar 3 — P2 call: E-F rise to a held G
    [(64, 2, 1, None, "mordent"), (65, 2, 0, None, None),
     (67, 8, 0, None, "flick"), (None, 2, 0, None, None),
     (67, 2, 0, None, None)],
    # bar 4 — P2 answer: the first melisma pours Ab-G-F onto held E
    [(68, 2, 1, 67, None), (67, 2, 0, 65, None), (65, 2, 0, 64, None),
     (64, 6, 0, None, None), (None, 2, 0, None, None),
     (61, 1, 0, None, None), (64, 1, 0, None, None)],
    # bar 5 — P3 call: F-G rise to a held Ab (the b6 color)
    [(65, 3, 1, None, "mordent"), (67, 1, 0, None, None),
     (68, 8, 0, None, "flick"), (None, 2, 0, None, None),
     (68, 2, 0, None, None)],
    # bar 6 — P3 answer: Bb-Ab-G melisma turns and lifts to held Bb
    [(70, 2, 1, 68, None), (68, 2, 0, 67, None), (67, 2, 0, 68, None),
     (68, 2, 0, None, None), (70, 6, 0, None, None),
     (None, 2, 0, None, None)],
    # bar 7 — P4 call: circling C' under the hang, leaning Db'
    [(72, 3, 1, None, "mordent"), (70, 1, 0, None, None),
     (72, 4, 0, None, None), (73, 2, 0, 72, None), (72, 2, 0, 70, None),
     (70, 2, 0, None, None), (68, 1, 0, None, None),
     (70, 1, 0, None, None)],
    # bar 8 — THE HANG: portamento up into the held Db5
    [(72, 2, 1, 73, None), (73, 10, 1, None, None),
     (None, 2, 0, None, None), (72, 1, 0, None, None),
     (73, 1, 0, True, None)],
    # bar 9 — P5: the answer opens at the top (entered by the hang's
    # cross-barline slide — that portamento IS the head's ornament),
    # falls to the held Ab whose flick owns its entry
    [(72, 3, 1, None, None), (73, 1, 0, None, None),
     (72, 2, 0, 70, None), (70, 2, 0, None, None),
     (68, 6, 0, None, "flick"), (None, 2, 0, None, None)],
    # bar 10 — P5 answer: G turns, melisma G-F onto held E
    [(67, 2, 1, None, "mordent"), (68, 2, 0, None, None),
     (67, 2, 0, 65, None), (65, 2, 0, 64, None),
     (64, 6, 0, None, None), (None, 2, 0, None, None)],
    # bar 11 — P6: the Db-E winding sung slow (the mode's heart)
    [(65, 2, 1, None, None), (64, 2, 0, None, None),
     (61, 2, 0, None, "flick"), (64, 2, 0, None, None),
     (61, 3, 0, 60, None), (60, 3, 0, None, None),
     (None, 2, 0, None, None)],
    # bar 12 — P6 answer: dips below home, Bb3 to held Ab3
    [(60, 2, 1, None, "mordent"), (61, 2, 0, None, None),
     (60, 4, 0, None, None), (58, 2, 0, None, None),
     (56, 4, 0, None, "flick"), (None, 2, 0, None, None)],
    # bar 13 — P7: from the depth a melisma climbs to held Db
    [(55, 2, 1, None, "mordent"), (56, 2, 0, 58, None),
     (58, 2, 0, 60, None), (60, 2, 0, 61, None),
     (61, 6, 0, None, None), (None, 2, 0, None, None)],
    # bar 14 — P7 answer: the home call echoed (bar 1 remembered)
    [(60, 3, 1, None, None), (61, 1, 0, None, None),
     (64, 6, 0, None, "flick"), (None, 2, 0, None, None),
     (64, 2, 0, None, None), (61, 2, 0, None, None)],
    # bar 15 — P8: the long descent melisma into the depth
    [(60, 2, 1, None, "mordent"), (61, 2, 0, 60, None),
     (60, 2, 0, 58, None), (58, 2, 0, 56, None),
     (56, 4, 0, None, None), (55, 2, 0, None, None),
     (None, 2, 0, None, None)],
    # bar 16 — THE CLOSE: the held Db->C, landing across the barline
    [(61, 2, 1, None, None), (None, 1, 0, None, None),
     (49, 1, 0, None, None), (61, 12, 0, 60, None)],
]
SONG_BARS = [[list(e) for e in bar] for bar in SONG_BARS]
for _bar in SONG_BARS:
    assert sum(e[1] for e in _bar) == 16
...
THEME_BARS = resolve_slides(SONG_BARS)
THEME = [tuple(e) for bar in THEME_BARS for e in bar]
assert sum(d for _, d, _, _, _ in THEME) == 256

# v3 invariant: an ornament never sits on a slide TARGET — the
# incoming portamento owns that attack (one attack per gesture).
_prev_target = None
for _m, _d, _a, _sl, _orn in THEME:
    if _m is not None:
        assert not (_orn is not None and _prev_target == _m), \
            "ornament on a slide target"
        _prev_target = _sl if isinstance(_sl, int) else None
    else:
        _prev_target = None
...
# partials, sine body core, within-note sweep) — but v3 renders it
# LEGATO: a slide chain is ONE note travelling a smoothed pitch path
# (one attack, one sweep, the VCA stays open — the true 303 tie), and
# ornaments are pitch INFLECTIONS inside that attack. Dry, center.
...
def place_events(b0, events, octave, base_cut, gain, prof):
    """The wire, sung legato: consecutive slid notes and their target
    merge into one acid_path call; a terminal slide (no next note)
    LANDS — its arrival pitch sounds ~100 ms across the line;
    detached single notes gate at 0.85 of the slot (air between)."""
    cuts = []
    evs = list(events)
    steps = []
    s = 0
    ...
    while i < len(evs):
        ...
        if orn == "flick" and seg[0][1] > 0.13:
            seg[0][1] -= 0.065
            seg.insert(0, [scale_below(m) + octave, 0.065])
        elif orn == "mordent" and seg[0][1] > 0.18:
            seg[0][1] -= 0.12
            seg.insert(0, [scale_above(m) + octave, 0.060])
            seg.insert(0, [m + octave, 0.060])
        x = acid_path(tuple(tuple(e) for e in seg), cut,
                      accent=bool(acc))
        add_at(lay_L, x, t0, g)
...
OUT = os.path.join(OUT_DIR, "morgenland_v3.wav")
...
FLAC = os.path.join(OUT_DIR, "morgenland_v3.flac")
...
# the melody map + metrics, all from THEME itself. v2: the SUNG
# metrics — density window, run ceiling, held fraction, breaths,
# melisma over any note length (the portamento is sung, not 16th).
chars = ["."] * 256
pos = 0
SOUNDING = 0
HELD_STEPS = 0
...
print(f"  onsets {ONSETS}/256 (density {onset_density:.2f}, "
      f"{ONSETS / (16 * BAR):.1f} notes/s), rests {RESTS} in {BREATHS} "
      f"breaths, longest 16th run {longest_run}, chains(2+) {chains2}, "
      f"melisma(3+) {chains3}, held fraction {held_frac:.2f}")
print(f"  hijaz crossings (Db<->E) {CROSSINGS}, flicks {FLICKS}, "
      f"mordents {MORDENTS} ({ORN_SHORT} on notes < an 8th), "
      f"accents {ACC_COUNT} ({ACC_OFF_SCALE} off-scale)")
...
checks = [
    ...
    (f"anti-arc: no brightness trend (slope {slope:+.2f} Hz/bar)",
     abs(slope) < 1.0),
    (f"sings-not-runs: onset density {onset_density:.2f} in 0.20-0.50 "
     f"({ONSETS / (16 * BAR):.1f} notes/s — a singable pace)",
     0.20 <= onset_density <= 0.50),
    (f"sings-not-runs: longest 16th run {longest_run} <= 6 "
     "(no silver_wire runs in the refrain)", longest_run <= 6),
    (f"sings-not-runs: held fraction {held_frac:.2f} >= 0.50 "
     "(steps in notes >= a dotted 8th)", held_frac >= 0.50),
    (f"breathes: {BREATHS} breath rests >= 6, rests {RESTS} in 16-48",
     BREATHS >= 6 and 16 <= RESTS <= 48),
    (f"melisma: slide chains(2+) {chains2} >= 4", chains2 >= 4),
    (f"ornaments sit on held notes: {ORN_SHORT} on notes < an 8th == 0",
     ORN_SHORT == 0),
