#!/usr/bin/env python3
"""
maschinenherz_v2.py — "Maschinenherz" v2 (~7:25, 145 BPM, E minor).
V2 = v0.3 + THE BIG-ROOM MASTER, NOTHING ELSE (notes:
maschinenherz_notes.md, v2 amendment). Not one note, bar or pattern
changed; the four mix moves are the verified fall_of_arrakeen chain:
sidechain pump on bass+pads+drone (55 % duck per kick, floor 0.30),
the sub-boom layer an octave under the bass root on every
4-on-the-floor kick, master shelves (high 3 kHz / low 95 Hz / deep
55 Hz) + the tanh bus limiter, and the wider pad spread. The 303,
the voice and the arp are NOT pumped — the singers keep their own
rhythm against the breathing bed. One declared fix: the shiver
kick-roll build at bar 19 was dead code in v0.3 (kick_on gated it
out); it now plays as the seam checklist always claimed.
...
Output: /workspace/music/maschinenherz_v2.wav + maschinenherz_v2.flac.
"""
...
def kick_on(b):
    s = section_of(b)
    if s == "shiver":
        return b == B_SHIVER + 15   # v2 fix: the kick-roll build plays now
...
# v2 — sidechain pump (the fall_of_arrakeen recipe): every
...
commit(lay_L, lay_R, 0.24, env=pump)   # v2: pumped — the kick owns the low end
print("psy bass committed")


# ------------------------------------------------------------- sub boom
# v2 — the fall_of_arrakeen wall-vibrator: a pure sine ONE OCTAVE
# UNDER the bass root (E1/C1/D1/B0) on every 4-on-the-floor kick,
# sustaining the beat, hard release in the last 60 ms so booms never
# overlap. Its OWN layer: peak normalization can't trade the sub
# against the punch. Follows bass_root(b), so the walking choruses
# keep their harmony in the sub; scaled by kick_gain(b).

def make_sub_boom(f):
    n = int(0.40 * SR)                          # beat at 145 is 0.414 s
    td = np.arange(n) / SR
...
commit(lay_L, lay_R, 0.13, env=pump)   # v2: pumped
...
commit(lay_L, lay_R, 0.07, env=pump)   # v2: pumped
...
# ---------------------------------------------------------------- master
# v2 — the fall_of_arrakeen chain: shelves (high + low + deep), then
...
# NO deep 55 Hz shelf (declared deviation from the fall chain): the
# boom layer already fills 31-41 Hz — the compound shelf measured a
# 0.65-0.77 sub-60 share and buried the mids/top under the sub.
...
OUT = os.path.join(OUT_DIR, "maschinenherz_v2.wav")
...
FLAC = os.path.join(OUT_DIR, "maschinenherz_v2.flac")
...
# v2 — big-room metrics: band shares of the final master + the pump
...
for (name, b0), (_, b1) in zip(SECTIONS, SECTIONS[1:] + [("end", None)]):
    ...
    print(f"  {name:22s} sub {SHARES[name][0]:.2f}   hf {SHARES[name][1]:.2f}")
...
checks = [
    ...
    ("K-b-b-b contract: zero bass hits on kick 16ths",
     BASS_GAP_VIOLATIONS == 0),
    (f"big-room: the drops carry the sub (drop1 {SHARES['DROP 1 (machine)'][0]:.2f}, "
     f"drop2 {SHARES['DROP 2 (FUSION)'][0]:.2f} >= 0.50)",
     SHARES["DROP 1 (machine)"][0] >= 0.50 and
     SHARES["DROP 2 (FUSION)"][0] >= 0.50),
    (f"big-room: the top octave is lit (drop1 hf {SHARES['DROP 1 (machine)'][1]:.2f}, "
     f"drop2 hf {SHARES['DROP 2 (FUSION)'][1]:.2f} >= 0.04)",
     SHARES["DROP 1 (machine)"][1] >= 0.04 and
     SHARES["DROP 2 (FUSION)"][1] >= 0.04),
]
print("\nForm checks:")
ok = True
