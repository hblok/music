#!/usr/bin/env python3
"""
silver_wire_v3.py — "Silver Wire" v3 (~5:38, 142 BPM, A minor).
V3 = V2 + THE BIG-ROOM MASTER, NOTHING ELSE (notes:
silver_wire_v2_notes.md, v3 amendment). Not one note, bar or pattern
changed; the four mix moves are the verified fall_of_arrakeen chain:
sidechain pump on bass+pads (55 % duck per kick, floor 0.30), the
sub-boom layer at the bass root under every 4-on-the-floor kick,
master shelves (high 3 kHz / low 95 Hz / deep 55 Hz) + the tanh bus
limiter, and the wider pad spread. The 303 is NOT pumped and gets no
delay — it stays bone dry and center; the world pumps around it.
...
Output: /workspace/music/silver_wire_v3.wav + silver_wire_v3.flac.
"""
...
# v3 — sidechain pump (the fall_of_arrakeen recipe): every
# 4-on-the-floor kick ducks the sustained layers ~55 % and lets them
# swell back over the beat. The kick owns the low end; the pumping IS
# the big-room breath. Roll bars don't pump (they are crescendos).
pump = np.ones(N)
_dipn = int(0.30 * SR)
...
commit(lay_L, lay_R, 0.28, env=pump)   # v3: pumped — the kick owns the low end
print("bass committed (roll outside the drops, sub duty inside)")


# ------------------------------------------------------------- sub boom
# v3 — the fall_of_arrakeen wall-vibrator: a pure sine at the bass
# root (A1/F1/G1/E1 — already the boom register) ON every
# 4-on-the-floor kick, sustaining the beat, hard release in the last
# 60 ms so booms never overlap. Its OWN layer: peak normalization
# can't trade the sub against the punch. Inside the drops it
# completes the low line — boom on the kick 16th, sub_note on the
# three after; the K-b-b-b contract is intact (the boom is the
# KICK's low end, not a bass onset).
...
BOOM = {m: make_sub_boom(midi_to_hz(m)) for m in set(BASS_LAP)}
...
commit(lay_L, lay_R, 0.14)     # modest: the sub-duty bass already owns 55 Hz
...
commit(lay_L, lay_R, 0.16, env=pump)   # v3: pumped; 0.13 -> 0.16 gives back
print("pads committed")                # the pump's average cut (~0.88)
...
# ---------------------------------------------------------------- master
# v3 — the fall_of_arrakeen chain: shelves (high + low + deep), then
# the gentle tanh bus limiter — glue, loudness, and transients can't
# steal normalization headroom from the drops.
...
# NO deep 55 Hz shelf (declared deviation from the fall chain): this
# bass already lives at 41-55 Hz — the compound shelf measured a
# 0.73-0.79 sub-60 share and buried the mids/top under the sub.
...
OUT = os.path.join(OUT_DIR, "silver_wire_v3.wav")
...
FLAC = os.path.join(OUT_DIR, "silver_wire_v3.flac")
...
# v3 — big-room metrics: band shares of the final master + the pump
...
# The summit comparison lives ABOVE the sub floor (v3 amendment): the
# two drops share one identical kick/bass/boom floor by design, so
# total RMS only ever measured the added voices — and the v3 boom +
# low shelf swamped that signal. >120 Hz is where the voices live.
...
checks = [
    ...
    (f"drop 2 (the summit run) out-sings drop 1 above the sub floor "
     f"(>120 Hz RMS {D2_HP:.3f} > {D1_HP:.3f}; declared, v3 amendment)",
     D2_HP > D1_HP),
    ...
    (f"big-room: the drops carry the sub (drop1 {SHARES['DROP 1 (pedal)'][0]:.2f}, "
     f"drop2 {SHARES['DROP 2 (summit)'][0]:.2f} >= 0.55)",
     SHARES["DROP 1 (pedal)"][0] >= 0.55 and
     SHARES["DROP 2 (summit)"][0] >= 0.55),
    (f"big-room: the top octave is lit (drop1 hf {SHARES['DROP 1 (pedal)'][1]:.2f}, "
     f"drop2 hf {SHARES['DROP 2 (summit)'][1]:.2f} >= 0.04)",
     SHARES["DROP 1 (pedal)"][1] >= 0.04 and
     SHARES["DROP 2 (summit)"][1] >= 0.04),
]
print("\nForm checks:")
ok = True
