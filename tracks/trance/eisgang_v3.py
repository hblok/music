#!/usr/bin/env python3
"""
eisgang_v3.py — "Eisgang" v3 (~4:55, 138 BPM, F minor). Hard
techno-trance in the CIRCLE form (design notes: eisgang_notes.md + its
v2/v3 amendments; inspiration: W3 "stamping feet" from Vivaldi's
Winter, ../../inspiration/Winter_Vivaldi.md — behaviors, no pitches).

v3 — ONE change from eisgang_v2.py: the melody instrument. Everything
else is identical (bass, drums, stab, structure, the underglow, and
every NOTE of the melody — the v2 phrase design with its stones,
ladder pickups, sighs and development levels was the review's keeper:
"the melody works really well"). The v2 synth piano still read as a
xylophone — "pling-plong", "far too timid" — and so did v1's hollow
pulse before it: the diagnosis is the ATTACK. Percussive decaying hits
read as toys regardless of timbre. The skyline now speaks on THE
SKYLINE LEAD: a sustained warm dark saw ensemble (the repo's proven
warm-lead family, dialed darker — LP 2200, 1/k**1.35, sine body, slow
bloom attack, FULL sustain for every note's length). Authority, not
pling.

The v2 changes this builds on (from eisgang.py): the refrain develops
by two Winter MECHANISMS (the second sanctioned borrow — mechanisms,
never pitches): the LADDER (W5 b3-4: climb chord tones into each stone
as a pickup) and the SIGH (W5 b5-6: dip to the diatonic lower neighbor
and return). Chorus 1 plain stones + sighs, chorus 2 + ladder pickups,
final chorus + octave dyads and answered sighs. Printed as the
development map, checked never-decreasing.
...
Output: /workspace/music/eisgang_v3.wav + eisgang_v3.mp3 (192k).
"""
...
# -------------------------------------- the skyline lead + the underglow
# THE WARM, re-voiced AGAIN for v3 — chorus-only. Two voices failed the
# listen test the same way (v1 hollow pulse, v2 synth piano: both "a
# xylophone", "pling-plong", "timid") and the diagnosis is the ATTACK,
# not the pitch material: percussive decaying hits read as toys. The
# melody itself was the keeper. So v3 keeps every note of the v2 phrase
# (stones, ladders, sighs, the development levels) and re-voices it on
# a SUSTAINED warm dark saw ensemble — the repo's proven warm-lead
# family (lost/farlight school per instruments/README.md, a shared
# family, not an owned signature), dialed darker: three detuned copies,
# 1/k**1.35 roll-off, sine body, LP 2200, a slow bloom attack and FULL
# sustain for the note's length. Authority instead of pling. The
# UNDERGLOW stays underneath (the v1 glide an octave down, LP 1600).
...
def pno(midi, b, beat, dur_beats, gain):
    x = skyline_note(midi, dur_beats * BEAT)
...
def skyline_statement(b0, level, gain=1.0):
...
def place_statement(b0, level, gain=1.0, glow=0.5):
    ...
    skyline_statement(b0, level, gain)
...
print(f"skyline lead + underglow committed "
      f"({len(skyline_cache)} notes cached)")
...
OUT = os.path.join(OUT_DIR, "eisgang_v3.wav")
...
MP3 = os.path.join(OUT_DIR, "eisgang_v3.mp3")
...
ham_cov = 100.0 * len(STAB_HAMMER_BARS) / len([b for b in CELL_BARS
                                               if BARTYPE[b] == "A"])
print(f"Skyline: lead statements {len(LEAD_STMTS)} at bars {LEAD_STMTS} "
