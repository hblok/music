#!/usr/bin/env python3
"""
morgenland_v2.py — "Morgenland" v2 (~6:12, 142 BPM, C Phrygian dom.).
...
SONG_BARS = [
    ...
    [(72, 2, 1, 73, None), (73, 10, 1, None, None),
     ...
     (73, 1, 0, True, None)],
    # bar 9 — P5: the answer opens at the top, melisma down to held Ab
    [(72, 3, 1, None, "mordent"), (73, 1, 0, None, None),
     (72, 2, 0, 70, None), (70, 2, 0, 68, None),
...
def acid_note(m, cutoff, accent=False, slide_to=None, dur=STEP * 0.92):
    ...
    # v2 — the sung vibrato: notes past a quarter breathe (+-0.6 %,
    # blooming after 0.35 s). The 303 sings; a dead-flat held tone
    # reads as an organ, not a voice.
    if dur > 0.30:
        vib = 1.0 + 0.006 * np.sin(2 * np.pi * 5.3 * td) * \
            np.clip((td - 0.35) / 0.4, 0, 1)
        fc_v = np.gradient(ph) * SR / (2 * np.pi) * vib
        ph = 2 * np.pi * np.cumsum(fc_v) / SR
    x = np.zeros(n)
...
OUT = os.path.join(OUT_DIR, "morgenland_v2.wav")
...
FLAC = os.path.join(OUT_DIR, "morgenland_v2.flac")
