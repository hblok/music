#!/usr/bin/env python3
"""Reliquary (Part 1) v2 — the dark cut.

v1's verdict (2026-09-05): structure, pads and hook placements good, but
the melody "still hasn't left Frankfurt": a tenor arch to C5/D5 with
vibrato and chorus over an updown arp and the Am-F-G lift.  v2 keeps the
form and the pads and changes the melodic VOCABULARY to early dark goth
EBM: baritone register (A2-Bb3), chant-like repeated notes, the flat 2nd
and flat 6th as the colour, a descending contour landing on the LOW A,
a hollow square voice with an octave-below chest, no chorus shimmer,
drier; the arp a one-octave DESCENDING sequence; the flat-VII chord in
the loop becomes the minor v (no lift); the riser becomes a downsweep.
Notes: reliquary_notes.md, "v2 amendment".  Same seed, same bars.
v3 (this script, 2026-09-05): the chest was too thin — the octave-below
...
# The dark hook: recitation on A3, the flat 2nd (Bb3) as the neighbour,
# the flat 6th (F3) falling to the 5th, phrase ends falling; Q hangs on
# the low E3, A lands on the LOW tonic A2 (doubling the bass for a bar).
HOOK = ["A3 - A3 A3 Bb3 - A3 -", "G3 - - - . A3 G3 F3", "E3 - F3 E3 D3 - E3 -", "E3 - - - - - . .",
        "A3 - A3 A3 Bb3 - A3 -", "G3 - - - . F3 E3 F3", "G3 - F3 E3 C3 - - -", "A2 - - - - - . ."]
TAG = ["A3 - G3 - E3 - - -", "- - - - - - . ."]
ARP_CUTOFF = {"BED": 350.0, "GROOVE": 500.0, "HOOK 1": 600.0, "HOOK 2": 600.0, "OUT": 400.0}
...
def chord_at(bar):
    if bar < 8:
        return AM
    if bar < 32:
        return (AM, F, EM, AM)[(bar - 8) % 4]        # v2: the minor v instead of the flat-VII lift
    if bar < 40:
        return (AM, F, AM, EM)[(bar - 32) % 4]
    return EM
...
def place_line(lines, bar0, count=True, chest=0.8):
    global HOOKS
    for start, midi, ln in parse(lines):
        t = bar_t(bar0) + start * BEAT / 2
        dur = ln * BEAT / 2 * 0.95
        place(LAYERS["lead"], dark_lead(midi, dur, chest), t)
...
for t, what in ((bar_t(4), "arp in (dark)"), (bar_t(8), "808 + bass + hit 1"),
                (bar_t(24) - BEAT / 2, "lead pickup (E3)"),
                (bar_t(32), "hit 2 + the downsweep; strings JOIN the pad; chest 1.0; harmony Am-F-Am-Em"),
...
for b0, device in ((8, "808 + bass enter ON the bar with hit 1"), (24, "lead pickup E3 on the & of 4"),
                   (32, "hit 2 + the downsweep falling away"), (40, "the ringing Em strings + the lead tag across the cut")):
...
check("last lead note is an E (any octave)", last_lead % 12 == 4)
...
steps = [b - a for a, b in zip(midis[:-1], midis[1:])]
downs = sum(1 for d in steps if d < 0) / max(1, sum(1 for d in steps if d != 0))
flat2 = sum(1 for m in midis if m % 12 == 10)
print(f"  DARK: register {min(midis)}-{max(midis)} (median {int(np.median(midis))}, ceiling Bb3=58)  "
...
check("Q hangs on an E, A lands on an A", q_end[1] % 12 == 4 and a_end[1] % 12 == 9)
check("baritone register: ceiling <= Bb3", max(midis) <= 58)
check("descending contour: down-steps >= 0.5", downs >= 0.5)
check("no soaring leap: max upward <= 5 st", max(steps) <= 5)
check("the flat 2nd colours the hook", flat2 >= 2)
