#!/usr/bin/env python3
"""No Access v3 — the jackhammer (tracks/ebm, 2026-09-11).  Working title Clearance.

v3, from the v2 listen verdict ("v2 is pretty good", but the bass
line "becomes very monotone ... it really drags", and the piece "starts
abruptly"):
  * the engine is a PHRASE, not one cell — an 8-bar shape (three bars
    of gallop, an octave bar, two gallop, a flat-7 bar, then a walking
    answer that climbs into the next phrase), a 4-bar filter cycle,
    16th accents, and a verse pedal that answers VI - VII every 8 bars;
  * a two-bar way in before bar 0: the bed swells, a low organ i, one
    slow noise swell — the gallop then ENTERS instead of starting.
Everything else is v2 (below).
...
# ------------------------------------------------------------- the piece
NAME = "no_access_v3"                                 # bump per iteration (no_access_v3.1 ...)
TOTAL_BARS = 160
INTRO_BARS = 2                                        # v3: the way in, before bar 0
END = (TOTAL_BARS + INTRO_BARS) * BAR + 0.5
N = int(END * SR)
SECTIONS = [("INTRO", -INTRO_BARS, 16), ("VERSE 1", 16, 32), ("PRE 1", 32, 40), ("CHORUS 1", 40, 56),
            ("VERSE 2", 56, 72), ("PRE 2", 72, 80), ("CHORUS 2", 80, 96), ("BREAK", 96, 112),
            ("FINAL", 112, 144), ("OUTRO", 144, 160)]

CS3, A2, GS2, B2 = 49, 45, 44, 47
...
CELL = {**CELLS, "8ths": "x.x.x.x.x.x.x.x.", "answer": "x.x.x.x.7.7.o.5.",
        # v3, the engine's phrase bars; every variant starts on the root (the chorus ledger)
        "lift": "x.xxx.xxo.xxo.xx",                   # octaves on 3 and 4
        "seven": "x.xxx.xx7.xx5.xx",                  # the flat 7, then the fifth
        "walk": "x.xxx.xx5.xx7.oo"}                   # the climb into the next phrase
INTERVAL = {"x": 0, "o": 12, "5": 7, "7": 10}
PHRASE = ("gallop", "gallop", "gallop", "lift", "gallop", "gallop", "seven", "walk")
BASS_CUT = (2400.0, 1700.0, 2900.0, 2000.0)           # the filter talks: one open value per two bars
VERSE_ROOTS = (CS3,) * 6 + (A2, B2)                   # the pedal answers VI - VII every 8 bars
BASS_ACCENT = {0: 1.0, 4: 0.92, 8: 1.0, 12: 0.92}     # the quarters lean, the 16ths sit back
...
def bar_t(b):
    return (b + INTRO_BARS) * BAR             # bar 0 sits INTRO_BARS in: the way in plays first
...
def bass_spec(b):
    ...
    cell = PHRASE[b % 8]                                      # v3: the 8-bar phrase, aligned to the chorus loop
    cut = (BASS_CUT[(b // 2) % 4], 250.0)
    if is_chorusy(b):
        return cell, ROOT[chord_at(b)], {"cutoff": cut, "sub": 0.85}   # the deeper bass (Q12): the sub square up
    return cell, VERSE_ROOTS[b % 8], {"cutoff": cut, "sub": 0.6}
...
if want("bed"):
    ...
    n_in = int(bar_t(0) * SR)                         # v3: the bed swells through the way in
    BED[:n_in] *= 0.5 - 0.5 * np.cos(np.pi * np.arange(n_in) / n_in)
    layer = new_layer()
    place(layer, BED, 0.0)
    commit("bed", layer)
...
if want("organ"):
    layer = new_layer()
    place_stereo(layer, organ(ORGAN_I, INTRO_BARS * BAR, cutoff=500.0, depth=0.0), 0.0, 0.7)   # v3: the way in
...
if want("bass"):
    ...
    for b in range(TOTAL_BARS):
        ...
        for j, s in enumerate(onsets):
            ...
            g = BASS_ACCENT.get(s, 0.78) if cell[s] == "x" else 1.0   # the colour notes always lean
            place(layer, note(m, dur=gap * STEP * 0.5, **kw), bar_t(b) + s * STEP, g)
...
if want("boom"):
    layer = new_layer()
    BOOM = {r: make_sub_boom(midi_to_hz(r + BOOM_OCTAVE)) for r in {root for _, root in KICKS}}
    for t, root in KICKS:
...
if want("fx"):
    layer = new_layer()
    place(layer, noise_sweep(INTRO_BARS * BAR, 150.0, 2500.0, res=2.5), 0.0, 0.45)   # v3: the way in
    for b in RISERS:
...
events = [(-INTRO_BARS, "the way in: the bed swells, low organ i, one noise swell"),
          (0, "bed + the riff naked (gallop, dark)"), (4, "kick in"), (8, "slam + hats + hit 1"),
...
bed_rms = [float(np.sqrt(np.mean(BED[i:i + win] ** 2)))
           for i in range(int(bar_t(0) * SR), int(bar_t(FADE[0]) * SR) - win, win)]
print(f"  bed continuous bar 0 .. bar {FADE[0]} (the fade): min 0.25 s RMS {min(bed_rms):.4f}   "
      f"(bars {-INTRO_BARS}..0 are the swell, by design)")
...
print(f"  drum onsets {len(drum_events)}; first kick bar {kicks[0] / BAR - INTRO_BARS:.2f}; last drum bar {max(t for t, _ in drum_events) / BAR - INTRO_BARS:.2f}")
...
engine = [b for b in range(TOTAL_BARS) if is_chorusy(b) or is_versey(b)]
phrase_cells = [PHRASE[b % 8] if b not in (55, 95, 143) else "answer" for b in engine]
colour = sum(1 for b, s, m, _ in bass_events if m != bass_spec(b)[1])
roots = sorted({bass_spec(b)[1] for b in engine})
print(f"  the phrase: {' '.join(PHRASE)};  filter cycle {BASS_CUT} Hz per two bars;  "
      f"accents {BASS_ACCENT} (the rest 0.78)")
print(f"  engine bars {len(engine)}: {len(set(phrase_cells))} cells, {colour} non-root onsets, roots {roots}")
check("the engine is a phrase, not one cell (>= 4 cells over the verses and choruses)", len(set(phrase_cells)) >= 4)
check("the verse pedal moves (VI - VII every 8 bars)", VERSE_ROOTS[6:] == (A2, B2) and len(roots) >= 4)
...
print(f"  boom: {boom_count} hits an octave under the bass root "
      f"({', '.join(f'{midi_to_hz(r + BOOM_OCTAVE):.0f}' for r in sorted({root for _, root in KICKS}))} Hz)")
...
check("no drum onset at/after bar 152", last_drum < bar_t(152), f"(last drum bar {last_drum / BAR - INTRO_BARS:.2f})")
