#!/usr/bin/env python3
"""No Access — the jackhammer (tracks/ebm, 2026-09-06).  Working title Clearance.
...
# ------------------------------------------------------------- the piece
NAME = "no_access"                                    # bump per iteration (no_access_v1.1 ...)
TOTAL_BARS = 160
...
TICK_CELL, TICK_DENSE, TICK_SPARSE = ".x...x...x...x..", ".x...x.x.x...x.x", ".x.......x......"
...
VOICE_GAIN = 0.0                                      # v1: instrumental (Q14); set > 0 with takes in VOICE_DIR
VOICE_DIR = pathlib.Path("/workspace/music/vocals/no_access")
VOICE_SLOTS = [(39, "no_access"), (79, "no_access"), (104, "no_access"), (112, "access_granted")]
...
def tick(midi):
    """The counter-sequence voice: the SH-101 as a dry square blip."""
    return note(midi, dur=STEP * 0.35, cutoff=(6000.0, 1500.0), env=0.02, res=4.5, sub=0.0, wave="square")


def tick_phrase():
    """Four ticks on 16ths — the code typed on the keypad; the v1 stand-in
    for the spoken phrases (retriggered = denied, plain = granted)."""
    buf = np.zeros(int((4 * STEP + 0.1) * SR))
    for i, m in enumerate(TICK_PITCHES):
        add_at(buf, tick(m), i * STEP)
    return norm(buf)


def take(name):
    """A spoken take from VOICE_DIR, mono at SR — or None (instrumental)."""
    path = VOICE_DIR / f"{name}.wav"
    if VOICE_GAIN <= 0.0 or not path.exists():
        return None
...
LAYER_NAMES = ["bed", "pad", "organ", "stab", "ticks", "bass", "drums", "lead", "double", "hit", "voice", "fx"]
WETS = {"bed": 0.0, "pad": 0.35, "organ": 0.3, "stab": 0.2, "ticks": 0.1, "bass": 0.0, "drums": 0.06,
        "lead": 0.25, "double": 0.35, "hit": 0.3, "voice": 0.15, "fx": 0.3}
WEIGHTS = {"bed": 0.30, "pad": 0.20, "organ": 0.24, "stab": 0.20, "ticks": 0.16, "bass": 0.40, "drums": 0.44,
           "lead": 0.40, "double": 0.16, "hit": 0.28, "voice": 0.28, "fx": 0.14}
...
def commit(name, layer):
    """Normalise, reverb, weight, add to the mix; the layer is dropped after."""
    L, R = layer
...
if want("ticks"):
    ...
    for b in range(TOTAL_BARS):
        ...
        for s, ch in enumerate(cell):
            if ch != ".":
                place(layer, tick(TICK_PITCHES[k % 4]), bar_t(b) + s * STEP, pan=-0.3)
                tick_events.append((b, s))
                k += 1
    commit("ticks", layer)
...
if want("voice"):
    layer = new_layer()
    for b, name in VOICE_SLOTS:
        x = take(name)
        if x is not None:
            y = machine(x, ring_hz=midi_to_hz(CS3)) if name == "no_access" else machine(x)   # the grant cleaner
            retrig = b == 104
        else:
            y = tick_phrase()
            retrig = name == "no_access"                  # the code typed and refused; granted = plain
        if retrig:
            y = retrigger(y, STEP, 3, head=0.08)
        place(layer, y, bar_t(b), VOICE_GAIN if x is not None else 1.0)
        voice_events.append((b, name, "retrigger" if retrig else "plain", "take" if x is not None else "ticks"))
    commit("voice", layer)
...
events = [(0, "bed + the riff naked (gallop, dark)"), (4, "kick in"), (8, "slam + hats + hit 1"),
          (20, "the ticks enter (the code on the keypad)"), (32, "offbeat bass, chorused stabs, open hats"),
          (38, "roll + riser"), (39, "NO ACCESS (slot 1)"), (40, "hit; refrain x2, chest 1.0; the downsweep"),
          (55, "the bass turnaround (answer cell)"), (56, "verse 2: ticks dense, stab opens"),
          (79, "NO ACCESS (slot 2)"), (80, "hit; refrain x2, chest 1.2"), (96, "kick OUT: bed, organ i, the riff on dark 8ths, ticks sparse"),
          (104, "organ v; NO ACCESS retriggered (slot 3)"), (110, "roll + riser"), (SILENT[0], "THE SILENT BEAT"),
          (112, "ACCESS GRANTED (slot 4) + hit; final chorus, chest 1.3"), (128, "hit; the octave double + the Juno down-arp (the fusion)"),
          (144, "outro: the chorus instrumental, ticks back"), (152, "drums out: bed + riff + ticks"),
...
tick_ch = sum(1 for b, _ in tick_events if is_chorusy(b) and b < 144)
tick_vb = sum(1 for b, _ in tick_events if is_versey(b) or is_pre(b) or 96 <= b < 112)
...
print(f"  tick onsets: verses/pre/break {tick_vb}, choruses {tick_ch};  refrain onsets: choruses {lead_ch}, elsewhere {lead_vb}")
check("ticks never in a chorus; refrain never outside one", tick_ch == 0 and lead_vb == 0 and tick_vb > 0 and lead_ch > 0)

print("\n=== THE SLOT ===")
print(f"  VOICE_GAIN {VOICE_GAIN}  takes in {VOICE_DIR}: {[n for _, n in VOICE_SLOTS if (VOICE_DIR / f'{n}.wav').exists()] or 'none'}")
