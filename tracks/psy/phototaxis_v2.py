#!/usr/bin/env python3
"""
phototaxis_v2.py — "Phototaxis" v2 (~5:00). Old-school GOA, tracks/psy/.
Design doc: phototaxis_notes.md (v1 listen verdict + v2 plan, 2026-07-30).

147 BPM, F# natural minor, seed 1995. Revision of v1 (which was fun but
"timid / pling pling, no depth", with a 33 s beatless break that read as
two separate songs). v2 fixes both:

  * DEPTH — a warm, wide, evolving PAD BED now runs across the whole
    groove (v1 had a sustained bed only in the anthem waves, so the first
    2:43 had nothing under the plinks); the bass is fattened with a sub
    octave, and the boom sub sits in the verses too, not just the drops.
    The glints (the keeper) and the swarm/anthem melodies are unchanged.
  * SONG FORM (notes Q8 answer: soften the withhold) — the anthem is now
    stated QUIET + SOLO as an early thesis (~0:20), developed through the
    verses, restated identically at each chorus, and the payoff is a
    FULLER restatement — not a first hearing. The old beatless POOL is a
    BRIDGE that keeps a half-time pulse and carries a half-lit anthem
    across it, so the halves share material (no dead seam).
...
BASS_CELLS = {
    ...
    # bridge pulse: sparse, off the half-time kick steps {0,8}, 2 pitches
    "E": [(6, 30, .60), (14, 28, .50)],
    # F — dense root-relative (the climax cell, D's 12-onset sibling)
    "F": [(1, 0, .55), (2, 0, .80), (3, 0, .60), (5, 12, .60),
          (6, 0, .80), (7, 0, .60), (9, 7, .60), (10, 0, .80),
          (11, 0, .60), (13, 12, .60), (14, 0, .80), (15, 7, .65)],
}
...
# v2 SONG FORM: intro -> thesis (quiet solo anthem) -> verses developing
# the swarm -> choruses restating the anthem -> a BRIDGE that keeps a
# half-time pulse and a half-lit anthem across it -> the fullest
# restatement (the payoff) -> an OUTRO bookend (solo anthem after the
# machinery stops).  New per-wave field `pad` (the depth bed, on
# everywhere but the drone-breath OPEN); `khalf` = half-time kick.
WAVES = [
    ...
    dict(name="INTRO",  b0=4,   n=8,  kick=1, ohat=5, chat=0, sn=0,
         bass="A", fizz=[], glint=[], murk=0, harm="ground", pad=1),
    dict(name="THESIS", b0=12,  n=8,  kick=1, ohat=1, chat=1, sn=0,
         bass="D", fizz=[], glint=[], murk=0, harm="loop", pad=1),
...
TOTAL_BARS = 184
# Anthem statements — dicts (song form): the thesis is early & quiet-solo,
# the choruses restate identically, the bridge carries a half-lit phrase,
# the FINAL resolves, the bookend closes solo.  phrase=1 -> first phrase
# only (the half-lit carry); solo -> that wave suppresses the swarm.
STATEMENTS = [
    dict(bar=12,  gain=0.55, final=False, solo=True,  phrase=1),   # thesis
    dict(bar=60,  gain=1.00, final=False, solo=False, phrase=None),  # chor1
    dict(bar=92,  gain=1.00, final=False, solo=False, phrase=None),  # chor2
    dict(bar=120, gain=0.55, final=False, solo=True,  phrase=1),   # bridge
    dict(bar=140, gain=1.00, final=False, solo=False, phrase=None),  # final
    dict(bar=156, gain=1.00, final=True,  solo=False, phrase=None),  # RESOLVE
    dict(bar=176, gain=0.50, final=False, solo=True,  phrase=None),  # bookend
]
FINAL_BAR = next(s["bar"] for s in STATEMENTS if s["final"])
SOLO_BARS = {s["bar"] for s in STATEMENTS if s["solo"]}   # thesis/bridge/out
FORESHADOWS = [(44, "fizz"), (100, "glint")]    # 2 bars each
BUBBLE_BARS = [58, 90, 138]                     # rise ends on next chorus
MINI_DIP = (-1, -1)                             # unused in v2
STRIP_GLINT_FIZZ_AT = 164                       # CHORUSOUT thins the swarm
CHATTER_BARS = [0, 12, 35, 76, 108]             # punctuation bursts
BRIDGE_SILENCE_BAR = 131                        # the one composed drop-beat
BRIDGE_CHATTER = [117, 121, 125, 129]
BRIDGE_GLINT = [(118, 85), (122, 90), (126, 88)]

if PREVIEW == "harmony":
    WAVES = [dict(name="PRE-HARM", b0=0, n=16, kick=1, ohat=1, chat=1,
                  sn=1, bass="D", fizz=["f4a", "f4b"], glint=[], murk=1,
                  harm="loop", pad=1)]
    TOTAL_BARS = 16
    STATEMENTS = [dict(bar=0, gain=1.0, final=False, solo=False, phrase=None),
                  dict(bar=8, gain=1.0, final=True, solo=False, phrase=None)]
    FINAL_BAR, SOLO_BARS = 8, set()
    FORESHADOWS, BUBBLE_BARS, CHATTER_BARS = [], [], []
    BRIDGE_CHATTER, BRIDGE_GLINT = [], []
    STRIP_GLINT_FIZZ_AT, BRIDGE_SILENCE_BAR = 10 ** 9, -1
elif PREVIEW == "swarm":
    WAVES = [dict(name="PRE-SWARM", b0=0, n=8, kick=1, ohat=1, chat=1,
                  sn=1, bass="C", fizz=["f2b", "f3a"],
                  glint=["g2a", "g2b"], murk=1, harm="ground", pad=1)]
    TOTAL_BARS = 8
    STATEMENTS, FORESHADOWS, BUBBLE_BARS, CHATTER_BARS = [], [], [], []
    FINAL_BAR, SOLO_BARS = -1, set()
    BRIDGE_CHATTER, BRIDGE_GLINT = [], []
    STRIP_GLINT_FIZZ_AT, BRIDGE_SILENCE_BAR = 10 ** 9, -1
    ...
    for i in range(w["n"]):
        ...
        if w["bass"] and not bass_off:
            for s, m, g in BASS_CELLS[w["bass"]]:
                midi = root + m if w["bass"] in ("D", "F") else m
                place(LAYERS["bass"], bass_note(midi, SIXT), t0 + s * SIXT, g)
                bass_reg.append((bar, s, midi))

        # ---- pad bed (v2 depth): the warm floor under the whole groove,
        # per 2-bar block following the harm root (an octave up from bass);
        # off in the drone-breath OPEN and on the composed drop-beat.
        if w["pad"] and pos % 2 == 0 and bar != BRIDGE_SILENCE_BAR:
            pad_root = (root + 12) if w["harm"] == "loop" else (FS1 + 12)
            bright = 0.62 if w["name"] == "BRIDGE" else 1.0
            PL, PR = pad_bed(pad_root, 2 * BAR * 1.02, bright, seed=bar % 7)
        ...
        # ---- underglow (the anthem's own mid-register bed, per 2 bars;
        # off in the strip; carries the single E# under the final V)
        if w["harm"] == "loop" and pos % 2 == 0 and not stripped:
            midis = UNDERGLOW[pos]
            if FINAL_BAR >= 0 and FINAL_BAR <= bar < FINAL_BAR + 8 and pos == 6:
...
for st in STATEMENTS:
    b0s, g, final, phrase = st["bar"], st["gain"], st["final"], st["phrase"]
    events = ANTHEM if phrase is None else [e for e in ANTHEM if e[0] < 32]
    if final:
        events = events + [RESOLUTION]
    for s, m, dsteps in events:
        t = step_t(b0s) + s * SIXT
        dur = dsteps * SIXT * 0.98 + 0.02
        y = gurgle_note(m, dur, held=dsteps >= 8)
        place(LAYERS["gurgle"], y, t, g)
        gurgle_reg.append((b0s + s / 16.0, m, dsteps))

# ---- bridge bed: dark drone + a half-lit anthem already handled above;
# sparse glints + machine-elf chatter carry the pulse (never beatless)
...
# ---- sub-boom layer, root-tracking.  v2 depth: now in the GROOVE too
# (v1 was drops-only), gentler under the verses, full under the choruses.
NO_BOOM = {"OPEN", "BRIDGE", "PRE-SWARM"}
_booms = {}
for w in WAVES:
...
# ---- wet, commit, master.  v2: gurgle rings more (space), pad is the
# new warm floor (dark, pumped, felt not blaring), boom eased slightly
# now that it also sits under the verses.
WETS = dict(gurgle=0.45, glint=0.40, fx=0.30, glow=0.30, pad=0.30,
            fizz=0.12, murk=0.15, drone=0.15)
WEIGHTS = dict(kick=0.95, bass=0.98, boom=0.85, fizz=0.46, glint=0.26,
               murk=0.32, gurgle=0.80, glow=0.32, pad=0.44, drone=0.55,
               hats=0.20, snare=0.28, fx=0.35)
PUMPED = {"fizz", "murk", "glow", "pad", "drone"}
...
stem = "phototaxis_v2" + (f"_preview_{PREVIEW}" if PREVIEW else "")
...
print(f"anthem statements at bars {[s['bar'] for s in STATEMENTS]} "
      f"(thesis={STATEMENTS[0]['bar']}, final/resolve={FINAL_BAR}), "
      f"foreshadow fragments at {[b for b, _ in FORESHADOWS]} (2 bars each), "
      f"bubble-rises into bars {[b + 2 for b in BUBBLE_BARS]}, "
      f"bridge drop-beat bar {BRIDGE_SILENCE_BAR}, "
...
# v2: song form has no "machine-only" drop — every drop carries the
# anthem + pad on top BY DESIGN, so both floors are pinned at 0.45
# (the flightpath precedent for melody-carrying drops); both printed.
check("chorus1 sub-60 share in 0.45-0.75", 0.45 <= s1 <= 0.75, f"({s1:.2f})")
check("payoff sub-60 share in 0.45-0.75", 0.45 <= s2 <= 0.75, f"({s2:.2f})")
...
print("\n=== ANTHEM BLOCK (song form) ===")
first = STATEMENTS[0]["bar"]
check("thesis stated EARLY (< 20% in) — the hook up front",
      first / TOTAL_BARS < 0.20, f"(bar {first} = {first / TOTAL_BARS:.0%})")
check("refrain restated >= 4 times (identical melody)", len(STATEMENTS) >= 4,
...
fin = sorted(bf_m_d for bf_m_d in gurgle_reg if FINAL_BAR <= bf_m_d[0] < FINAL_BAR + 9)
res_ok = (fin[-1][1] == 66 and abs(fin[-1][0] - (FINAL_BAR + 8)) < 1e-6
          and fin[-2][1] == 73)
check("final statement resolves held C#5 -> F#4 across the barline",
      res_ok, f"(lands bar {fin[-1][0]:.0f}, from midi {fin[-2][1]})")
...
print("\n=== v2 DEPTH + SEAM (the listen-verdict fixes) ===")
...
print("\n=== SEAM / FX BLOCK ===")
check("bubble-rises land on chorus downbeats",
      [b + 2 for b in BUBBLE_BARS] == [60, 92, 140],
